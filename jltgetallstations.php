...
if(is_file($namelist) == FALSE)
	...
	for($i = 0; $i < count($bokstaver); $i++)
		...
		while($newdata)
			...
			$newdata = FALSE;
			$doc = new DOMDocument();
			@$doc->loadHTML(file_get_contents('http://193.45.213.123/jonkoping/indexes.aspx?optType=0&selKommun=0&sLetter='.urlencode(utf8_decode($bokstaver[$i])).'&iPage='.$page.'&Language=se&optFrTo=0&TNSource=JLT'));
